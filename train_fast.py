"""
Entrenamiento del Cubo de Rubik - Version optimizada con backend C

Este script usa el backend C vectorizado para entrenar a ~100k+ steps/segundo.

Uso:
    python train_fast.py                        # Entrenar con defaults
    python train_fast.py --num-envs 1024        # Mas entornos paralelos
    python train_fast.py --device cuda          # Usar GPU
    python train_fast.py --total-timesteps 50M  # Entrenar mas tiempo
"""
...
def layer_init(layer, std=np.sqrt(2), bias_const=0.0):
    """Inicializacion ortogonal para capas lineales (mejor para RL)."""
...
class RubikPolicy(nn.Module):
    ...
    def __init__(self, obs_size=324, action_size=12, hidden_size=512, num_blocks=4):
        super().__init__()
...
class PPOTrainer:
    """
    Proximal Policy Optimization optimizado para entornos vectorizados.
    """
...
def parse_timesteps(value):
    """Parse timesteps con sufijos K, M, B."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Entrenar Cubo de Rubik con backend C optimizado"
    )
    ...
    parser.add_argument("--total-timesteps", type=str, default="10M")
    ...
    args.total_timesteps = parse_timesteps(args.total_timesteps)
    ...
    # Create environment (C backend)
    env = RubiksCubeBatchEnvC(
    ...
    print(f"Entorno creado: {env.num_envs} entornos paralelos")
